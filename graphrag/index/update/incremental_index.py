...
async def update_dataframe_outputs(
    ...
    merged_entities_df, entity_id_mapping = await _update_entities(
        dataframe_dict, storage, update_storage, config, cache, callbacks, progress_logger=progress_logger
    )
...
async def _update_entities(
    dataframe_dict, storage, update_storage, config, cache, callbacks, progress_logger
):
